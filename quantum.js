...
async function fromANU() {
  ...
  // ANU Quantum Numbers(新版 AWS 服务)：
  //   length = 数组长度 1–1024；type = hex16；size = 每块字节数，hex16 时须为 1–10。
  //   length=8 & size=8 → 8 个块、每块 8 字节 = 128 个十六进制字符。
  const r = await fetch("https://api.quantumnumbers.anu.edu.au?length=8&type=hex16&size=8", {
